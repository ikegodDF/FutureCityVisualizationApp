...
class AnalysisService:
    ...
    def analyze(self, request: ComputeRequest):
        ...
        for i in range(5):
            result = self.compute_service.compute(ComputeRequest(method=request.method, appStateYear=request.appStateYear + 5*i, disasterState=request.disasterState, selectedRanges=request.selectedRanges, params=target_params))
